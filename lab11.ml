(*CSCI 2041 Lab11 Jiatan Huang && Ziyue Zhuang*)
type
  thing =
...
(* QUOTE. Return the single argument without evaluating it. *)
  ...
  primitive "is-atom"
    ...
    with Cons(arg, Nil) ->
          ...
          with  Nil -> Symbol "t"|
              Cons(_, Nil) -> Symbol "t"|
              Closure(_,_,_) -> Symbol"t"|
              Number n -> Symbol "t"|
              Primitive thing -> Symbol "t"|
              Symbol thing -> Symbol "t"|
              _ -> Nil)|
  ...
  let rec implying args =
    ...
    with Cons(Nil, Nil) ->
         ...
         Cons (arg, Nil) ->
           evaluating arg env |
         Cons (arg, args) -> implying args|
         _ ->
           oops "IMPLY expected one or more arguments"
  in implying args) ;;
  ...
  (fun args env ->
    match args
  with Cons(Symbol s, Cons(value, Cons(body, Nil)))->
    let newEnv = envPut s value env
  in evaluating body newEnv|
  _ -> oops "LET expected exactly three arguments");;
